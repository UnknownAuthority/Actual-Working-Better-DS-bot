...
class Mod(commands.Cog):
  ...
  async def kick(self, ctx, members: commands.Greedy[discord.Member], *, reason = None):
    ...
    for member in members:  
      ...
      #make an embed
      embed =  embed = embed=discord.Embed(title='Kicked Member', description=f'Kicked {member}')
  ...
  async def ban(self, ctx, members: commands.Greedy[discord.Member],deletedays: typing.Optional[int] = 1, *, reason: typing.Optional[str] = ''):
    ...
    for member in members:
      ...
      #make an embed
      embed = embed=discord.Embed(title='Banned Member', description=f'banned {member}')
      embed.set_author(name='BetterBot')
      embed.add_field(name='Reason', value=reason, inline=True)
      await ctx.send(embed=embed)
